...
from .errors import ConfigurationError, DefuzeError, ProviderError, ValidationError
...
from .repo_meta import collect_repo_meta
from .requirements import RequirementSpec, parse_requirement
from .run import Run
from .runtime import RuntimeSession
from .trace_evidence import TraceEvidenceCapture
from .tracking.evidence import EvidenceCollector
...
def configure(*, api_key: str) -> Path:
    ...
    ``DEFUZEX_CONFIG_HOME`` can redirect the user-level location for isolated
    ...
    *,
    ...
    if official_case or official_judge:
        backend = BackendClient(
            api_key,
            base_url=os.environ.get("DEFUZEX_BASE_URL") or DEFAULT_BASE_URL,
    ...
    if not config.judge:
        ...
        adapted_judge = OfficialJudgeProvider(
            ...
            allow_sensitive=config.allow_sensitive,
            operation_wait_timeout=config.operation_wait_timeout,
            state_root=repo_path / ".defuzex" / "runs",
        )
    else:
    ...
    *,
    ...
    if trace_evidence is not None and not isinstance(
        ...
        raise ConfigurationError(
            "trace_evidence must come from defuzex.otel.configure_trace_evidence()"
        )
    return config
...
def _evidence_collector(
    *,
    ...
    return EvidenceCollector(
        ...
        excluded_roots=(runtime.workspace.runtime_root, resolved_repo / ".defuzex"),
        track_files=config.track_files,
...
def create_run(
    *,
    ...
    be the capture returned by :func:`defuzex.otel.configure_trace_evidence`.
