...
| `operation_wait_timeout` | `float` | `600.0` seconds | Limits the total synchronous wait for one official Case or Judge operation, including polling. Polls start at the Backend `poll_after_ms` (or 1000 ms when resuming), grow geometrically toward 60s, and honor an optional revised `poll_after_ms` on queued/running GET responses. If it expires, KUMA raises a retryable timeout and keeps safe recovery metadata so the same operation can be resumed. |
