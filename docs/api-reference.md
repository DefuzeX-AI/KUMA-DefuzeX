...
| `operation_wait_timeout` | `float` | `600.0` seconds | Limits the total synchronous wait for one official Case or Judge operation, including polling. Polls start at the Backend `poll_after_ms` (or 1000 ms when resuming), grow geometrically toward an 8s local fallback cap when a GET omits the field, honor an optional revised `poll_after_ms` on queued/running GET responses within 100..60000 ms, and use remaining deadline for a final GET before raising a retryable timeout. If it expires, KUMA raises that timeout and keeps safe recovery metadata so the same operation can be resumed. |
...
these safe constraints in its English correction message.
...
`service_busy`, message `Service is busy. Please try again later.`, and no details. The original `retryable`
